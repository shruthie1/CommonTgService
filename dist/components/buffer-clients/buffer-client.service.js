...
let BufferClientService = class BufferClientService {
    ...
    async addNewUserstoBufferClients(badIds, goodIds) {
        ...
        const documents = await this.usersService.executeQuery({ "mobile": { $nin: goodIds }, expired: false, twoFA: false, lastActive: { $lt: sixMonthsAgo }, totalChats: { $gt: 300 } }, { tgId: 1 }, badIds.length + 3);
