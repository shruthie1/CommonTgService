import { BadRequestException, NotFoundException, OnModuleDestroy } from '@nestjs/common';
...
import { canonicalizeMobile, mobilesEqual } from './mobile-utils';
...
export abstract class BaseClientService<TDoc extends BaseClientDocument> implements OnModuleDestroy {
    ...
    protected isFrozenError(errorDetails: { message?: string; error?: unknown }): boolean {
    ...
    private extractConfigValue(node: unknown, targetKey: string): unknown {
    ...
    protected async buildPermanentAccountReason(baseReason: string, telegramClient?: TelegramManager | null): Promise<string> {
        ...
        try {
            ...
            const extras: string[] = [];
            if (Number.isFinite(freezeSinceNumber)) extras.push(`freeze_since=${new Date(freezeSinceNumber * 1000).toISOString()}`);
            if (Number.isFinite(freezeUntilNumber)) extras.push(`freeze_until=${new Date(freezeUntilNumber * 1000).toISOString()}`);
            if (typeof freezeAppealUrl === 'string' && freezeAppealUrl) extras.push(`appeal_url=${freezeAppealUrl}`);
    ...
    protected async performHealthCheck(mobile: string, lastChecked: number, now: number): Promise<{ passed: boolean; performed: boolean }> {
        ...
        try {
            ...
            if (hasPassword) {
                ...
                if (verificationStatus === 'ours') {
                    ...
                    this.botsService.sendMessageByCategory(
                        ChannelCategory.ACCOUNT_NOTIFICATIONS,
                        `<b>FOREIGN 2FA</b> ${this.clientType} ${doc.mobile} — unrecoverable, ${deactivated ? 'inactivated' : 'inactivate FAILED'}`,
                        { parseMode: 'HTML' }
                    );
                    return 0;
                }
    ...
    async processClient(doc: TDoc, client: Client): Promise<ProcessClientResult> {
        ...
        if (warmupAction.action === 'advance_to_ready') {
            ...
            this.botsService.sendMessageByCategory(
                ChannelCategory.ACCOUNT_NOTIFICATIONS,
                `<b>WARMUP READY</b> ${this.clientType} ${doc.mobile} — ${doc.channels || 0} channels, eligible for rotation`,
                { parseMode: 'HTML' }
            );
        ...
        try {
            ...
            switch (warmupAction.action) {
                ...
                case 'set_2fa':
                    ...
                    if (updateCount === 0) {
                        this.botsService.sendMessageByCategory(
                            ChannelCategory.ACCOUNT_NOTIFICATIONS,
                            `<b>WARMUP FAILED</b> ${this.clientType} ${doc.mobile}: set_2fa — ${failedAttempts + 1} fails`,
                            { parseMode: 'HTML' }
                        );
                    }
                ...
                case 'remove_other_auths':
                    ...
                    if (updateCount === 0) {
                        this.botsService.sendMessageByCategory(
                            ChannelCategory.ACCOUNT_NOTIFICATIONS,
                            `<b>WARMUP FAILED</b> ${this.clientType} ${doc.mobile}: remove_other_auths — ${failedAttempts + 1} fails`,
                            { parseMode: 'HTML' }
                        );
                    }
                ...
                case 'rotate_session':
                    ...
                    if (updateCount > 0) {
                        ...
                        this.botsService.sendMessageByCategory(
                            ChannelCategory.ACCOUNT_NOTIFICATIONS,
                            `<b>WARMUP FAILED</b> ${this.clientType} ${doc.mobile}: rotate_session — ${(doc.failedUpdateAttempts || 0) + 1} fails`,
                            { parseMode: 'HTML' }
                        );
                    }
            ...
            if (isPermanentError(errorDetails)) {
                ...
                this.botsService.sendMessageByCategory(
                    ChannelCategory.ACCOUNT_NOTIFICATIONS,
                    `<b>WARMUP PERMANENT ERROR</b> ${this.clientType} ${doc.mobile}: ${warmupAction.action} — ${deactivated ? 'inactivated' : 'inactivate FAILED'}\n${errorDetails.message?.substring(0, 120)}`,
                    { parseMode: 'HTML' }
                );
            } else {
                this.botsService.sendMessageByCategory(
                    ChannelCategory.ACCOUNT_NOTIFICATIONS,
                    `<b>WARMUP ERROR</b> ${this.clientType} ${doc.mobile}: ${warmupAction.action} — ${failCount}/${this.MAX_FAILED_ATTEMPTS} fails\n${errorDetails?.message?.substring(0, 120) || 'unknown'}`,
                    { parseMode: 'HTML' }
                );
            }
            return { updateCount: 0 };
    ...
    async getUsageStatistics(clientId?: string): Promise<{
        ...
        const filter: MongoQuery = { status: 'active' };
        if (clientId) filter.clientId = clientId;
