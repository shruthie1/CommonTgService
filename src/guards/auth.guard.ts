...
export class AuthGuard implements CanActivate {
    ...
    private extractRealClientIP(request: Request): string {
        ...
        if (request.connection?.remoteAddress) return request.connection.remoteAddress.replace('::ffff:', '');
    ...
    private extractRealOrigin(request: Request): string | undefined {
        ...
        if (xOriginalHost) return `${this.extractProtocol(request)}://${xOriginalHost}`;

        const xForwardedHost = this.getHeaderValue(request, 'x-forwarded-host');
        if (xForwardedHost) return `${this.extractProtocol(request)}://${xForwardedHost}`;
    ...
    private notifyUnauthorized(clientIp: string, origin: string | undefined, originalUrl: string) {
        try {
